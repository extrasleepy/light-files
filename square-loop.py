...
seedPin = AnalogIn(board.A2)
randnum = seedPin.value
random.seed(randnum)
...
def color_chase(color, wait):
    ...
    for i in range(grow, num_pixels):
        ...
        time.sleep(wait)
        pixels.show()
    for decend in range(decend, -1, -1):
        pixels[decend] = color
...
UNEXPECTED = (random.randint(0,255),255,random.randint(0,255))
